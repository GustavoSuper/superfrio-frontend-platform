...
export default function List_Compressor({ history }) {
...
async function loadProd() {
  ...
  //setProd(data);
  setProd(data.filter(item => item.modelo && item.modelo.toLowerCase().includes(busca.toLowerCase())))
  setLoading(false);
};
...
const goToPage = (state) => {
  history.push('/compressor/' + state._id);
};
...
const columns = [
  ...
  useEffect(() => {
    setLoading(true);
    loadProd();
    setMsgvazio("Nenhum compressor encontrado");
    setTimeout(() => {
    ...
    if(rowsSel.length > 0){
      if (window.confirm('Confirma remoção dos itens selecionados ?')){
        ...
        rowsSel.map(async (item) => {
            await api.delete('/compressor/' + item._id).then((res) => {
    ...
    <div>
        <div className="content-wrapper">
            ...
            <section className="content-header">
                <h1>
                    Modelos<small>( tabela modelos compressores )</small>
                </h1>
            </section>
            ...
            <section className="content">
                <div className="row">
                    <div className="col-xs-12">
                        ...
                        <button type="button" class="btn btn-success btn-flat margin" onClick={() => {history.push('/compressor/novo')}}>Novo Modelo</button>
