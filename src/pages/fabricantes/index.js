...
export default function List_Fabricantes({ history }) {
...
const goToPage = (state) => {
  history.push('/fabricante/' + state._id);
};
...
const handleChange = (state) => {
  ...
  useEffect(() => {
    setLoading(true);
    loadProd();
    setMsgvazio("Nenhum fabricante encontrado");
    setTimeout(() => {
    ...
    if(rowsSel.length > 0){
      if (window.confirm('Confirma remoção dos itens selecionados ?')){
        ...
        rowsSel.map(async (item) => {
            await api.delete('/fabricante/' + item._id).then((res) => {
    ...
    <div>
        <div className="content-wrapper">
            ...
            <section className="content-header">
                <h1>
                    Fabricantes<small>( tabela fabricantes )</small>
                </h1>
            </section>
            ...
            <section className="content">
                <div className="row">
                    <div className="col-xs-12">
                        ...
                        <button type="button" class="btn btn-success btn-flat margin" onClick={() => {history.push('/fabricante/novo')}}>Novo Fabricante</button>
