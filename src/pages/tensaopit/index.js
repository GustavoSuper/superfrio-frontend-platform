...
const goToPage = (state) => {
  history.push('/tensaopit/' + state._id);
};
      ...
      if(rowsSel.length > 0){
        if (window.confirm('Confirma remoção dos itens selecionados ?')){
          ...
          rowsSel.map(async (item) => {
              await api.delete('/tensaopit/' + item._id).then((res) => {
    ...
    <div>
        <div className="content-wrapper">
            ...
            <section className="content-header">
                <h1>
                    Tensão Pit Stop<small>( tabela tensão )</small>
                </h1>
            </section>
            ...
            <section className="content">
                <div className="row">
                    <div className="col-xs-12">
                        ...
                        <button type="button" class="btn btn-success btn-flat margin" onClick={() => {history.push('/tensaopit/novo')}}>Nova Tensão</button>
