...
export default function List_CategoriaDesp({ history }) {
...
async function loadProd() {
  const query = '/categoriadespesa';
  const response = await api.get(query);
  const data = await response.data;
  //setProd(data);
  setProd(data.filter(item => item.categoria && item.categoria.toLowerCase().includes(busca.toLowerCase())))
  setLoading(false);
}
...
const goToPage = (state) => {
  history.push('/categoriadesp/' + state._id);
};

  useEffect(() => {
    setLoading(true);
    loadProd();
    setMsgvazio("Nenhuma categoria encontrado");
    setTimeout(() => {
      ...
      if(rowsSel.length > 0){
        if (window.confirm('Confirma remoção dos itens selecionados ?')){
          ...
          rowsSel.map(async (item) => {
              await api.delete('/categoriadespesa/' + item._id).then((res) => {
    ...
    <div>
        <div className="content-wrapper">
            ...
            <section className="content-header">
                <h1>
                    Categorias<small>( tabela categorias de despesas )</small>
                </h1>
            </section>
            ...
            <section className="content">
                <div className="row">
                    <div className="col-xs-12">
                        ...
                        <button type="button" class="btn btn-success btn-flat margin" onClick={() => {history.push('/categoriadesp/novo')}}>Nova Categoria</button>
