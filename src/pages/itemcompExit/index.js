...
async function loadProd() {
  const query = '/checklistitemdefaultexit/type/compExit';
...
const goToPage = (state) => {
  history.push('/itemcompExit/' + state._id);
};
      ...
      if(rowsSel.length > 0){
        if (window.confirm('Confirma remoção dos itens selecionados ?')){
          ...
          rowsSel.map(async (item) => {
              await api.delete('/checklistitemdefaultexit/' + item._id).then((res) => {
    ...
    <div>
        <div className="content-wrapper">
            ...
            <section className="content-header">
                <h1>
                    Itens Saída Compressor<small>( tabela itens )</small>
                </h1>
            </section>
