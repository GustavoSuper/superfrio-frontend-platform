...
export default function List_ChecklistUnidade({ history }) {
  ...
  // );
    ...
    const goToPage = (state) => {
      history.push('/checklistrack/' + state._id);
    };

  async function loadProd() {
    const query = '/checklistrack';
  ...
  useEffect(() => {
    setLoading(true);
    setTimeout(() => {
      if(document.getElementById('menu_asunidades_consulta')){
        document.getElementById('menu_asunidades_consulta').className = "active";
      }      
    }, 50);
    loadProd();
    setTimeout(() => {
    ...
    if(rowsSel.length > 0){
      if (window.confirm('Confirma remoção dos itens selecionados ? Essa ação é irreversível')){
        ...
        rowsSel.map(async (item) => {
            await api.delete('/checklistrack/' + item._id).then((res) => {
    ...
    <div>
        <div className="content-wrapper">
            ...
            <section className="content-header">
                <h1>
                    CheckLists Unidades<small>( tabela checklist )</small>
                </h1>
            </section>
            ...
            <section className="content">
                ...
                <div className="row">
                    <div className="col-xs-12">
                        ...
                        <button type="button" class="btn btn-success btn-flat margin" onClick={() => {history.push('/checklistrackunidade/novo')}}>Novo Checklist Unidade</button>
