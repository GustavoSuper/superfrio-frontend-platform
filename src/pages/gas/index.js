...
export default function List_Gas({ history }) {
...
const handleChange = (state) => {
  ...
  useEffect(() => {
    setLoading(true);
    loadProd();
    setMsgvazio("Nenhum gas encontrado");
    setTimeout(() => {
    ...
    if(rowsSel.length > 0){
      if (window.confirm('Confirma remoção dos itens selecionados ?')){
        ...
        rowsSel.map(async (item) => {
            await api.delete('/gas/' + item._id).then((res) => {
            ...
            <section className="content">
                <div className="row">
                    <div className="col-xs-12">
                        ...
                        <button type="button" class="btn btn-success btn-flat margin" onClick={() => {history.push('/gas/novo')}}>Novo Gás</button>
